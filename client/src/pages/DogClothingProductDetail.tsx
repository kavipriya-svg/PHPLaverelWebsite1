...
import { ShoppingCart, ArrowRight, Star, Truck, RotateCcw, Shield, Timer, BadgeCheck, Heart } from "lucide-react";
...
// ─── Main Page ────────────────────────────────────────────────────────────────
export default function DogClothingProductDetail() {
  const [, params] = useRoute("/clothing/product/:slug");
  ...
  // Dog clothing testimonials (admin-managed editorial testimonials)
  ...
  const handleAddToCart = async () => {
    try {
      ...
      toast({ title: `${product.title} added to dossier` });
    ...
    <div className="flex gap-0.5">
      {[1, 2, 3, 4, 5].map((s) => (
        <svg key={s} width={14} height={14} viewBox="0 0 24 24"
          fill={s <= rating ? (light ? C.mint : C.primary) : "none"}
          stroke={light ? C.mint : C.primary} strokeWidth={1.5}>
          <polygon points="12,2 15.09,8.26 22,9.27 17,14.14 18.18,21.02 12,17.77 5.82,21.02 7,14.14 2,9.27 8.91,8.26" />
        </svg>
      ))}
    </div>
  ...
  return (
    <>
      ...
      <main style={{ marginTop: 96, backgroundColor: "#f9faf6", color: "#1a1c1a", fontFamily: "Inter, sans-serif", overflowX: "hidden" }}>
        ...
        <section className="grid grid-cols-1 md:grid-cols-12 gap-6 py-20 min-h-screen px-5 md:px-16">
            ...
            <div className="flex-grow">
              <div className="relative overflow-hidden group" style={{ aspectRatio: "4/5", ...HARD_SHADOW }}>
                ...
                {(product as any).isNewArrival ? (
                  ...
                  <div className="absolute bottom-6 left-6 px-4 py-1" style={{ backgroundColor: C.primary, color: "#fff", ...LABEL_CAPS, letterSpacing: "0.2em" }}>Urban Collection</div>
                )}
              </div>
              {allImgs.length > 1 && (
          ...
          <div className="md:col-span-5 flex flex-col justify-start pt-8 md:pt-24 space-y-8">
            <div className="space-y-2">
              <p style={{ ...LABEL_CAPS, color: C.secondary }}>Dog Clothing Archive / Urban Collection</p>
              ...
              <div className="flex flex-wrap items-center gap-4 py-3 border-y" style={{ borderColor: `${C.outlineVariant}33` }}>
                <span style={{ ...MONO, fontSize: 14, color: C.onSurface }}>
                  SPECIMEN NO. {specimenNo(product.id)}{product.sku ? ` | SKU: ${product.sku}` : ""}
                </span>
            ...
            <div className="space-y-8">
              ...
              {variants.length > 0 ? (
                <div className="space-y-4">
                  <label style={{ ...LABEL_CAPS, color: C.onSurfaceVariant, display: "block" }}>
                    Select {variants[0]?.optionName || "Size"} (SKU)
                  </label>
                ...
                <div className="space-y-4">
                  <label style={{ ...LABEL_CAPS, color: C.onSurfaceVariant, display: "block" }}>Size Guide</label>
                  <div className="grid grid-cols-2 gap-4">
                    <div className="p-4 flex justify-between items-center" style={{ border: `2px solid ${C.primary}` }}>
                      <span style={{ fontWeight: 700, color: C.primary }}>{weight ? `${weight}` : "Standard"}</span>
              ...
              <div className="p-4 flex items-center gap-4" style={{ border: `1px solid ${C.secondary}33`, backgroundColor: "#fe9e71" }}>
                ...
                <div>
                  ...
                  <p style={{ fontSize: 13, color: C.onSurfaceVariant, marginTop: 2 }}>Exclusive members-only pricing on all clothing.</p>
                </div>
              </div>
              ...
              {bannerUrl && (
                <div className="relative overflow-hidden">
                  {bannerCtaLink
                    ? <a href={bannerCtaLink}><img src={bannerUrl} alt={bannerTitle || ""} className="w-full object-cover max-h-40" />
                ...
                <div className="space-y-3 pb-2">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {visibleCoupons.map((c) => (
                      <button key={c.id} data-testid={`coupon-${c.id}`} onClick={() => copyCode(c.code)} title="Click to copy"
              ...
              <div className="flex flex-col gap-2">
                {[
                  ["Graphene-Sync Technology", "Thermal homeostasis at 38.5°C"],
                  ["Veterinary-Grade Textiles", "Tested for canine biometrics"],
                ].map(([label]) => (
        ...
        <AdBannerStrip banners={bs("bio", "bottom")} />
              ...
              <div className="grid grid-cols-1 md:grid-cols-12 gap-8 mb-16">
                ...
                <div className="md:col-span-9 flex flex-col justify-center">
                  ...
                  <p style={{ fontSize: 16, color: C.onSurfaceVariant, lineHeight: 1.6 }}>
                    Verified field reports from subjects in the urban environment. Unedited observations from real dogs, real results.
                  </p>
                </div>
              </div>
                  ...
                  <div key={t.id} className="flex flex-col" style={{ border: `1px solid ${C.outlineVariant}4D`, backgroundColor: "#fff" }}>
                    ...
                    <div className="flex flex-col gap-4 p-6">
                      ...
                      <div className="flex gap-4 flex-wrap pt-2 border-t" style={{ borderColor: `${C.outlineVariant}4D` }}>
                        {t.location && <span style={{ ...MONO, fontSize: 10, color: C.onSurfaceVariant }}>LOCATION: {t.location}</span>}
                ...
                return (
                  <Link key={p.id} href={`/clothing/product/${p.slug}`}>
                    <div className="group cursor-pointer" data-testid={`related-product-${p.id}`}>
                      ...
                      <p style={{ ...LABEL_CAPS, fontSize: 10, color: C.secondary, marginBottom: 4 }}>Dog Clothing</p>
