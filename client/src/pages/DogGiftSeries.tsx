...
import { ChevronDown, ShoppingBag, Menu, X, Search, ArrowRight } from "lucide-react";
import {
...
const DOSSIERS: Dossier[] = [
  {
    ...
    imageRight: false, bg: C.surfaceContainerLow, dark: false,
    ctaText: "Acquire Bundle", ctaStyle: "solid",
    specs: [
  ...
  },
  {
    ...
    imageRight: true, bg: C.surfaceContainerHighest, dark: false,
    ctaText: "Request Access", ctaStyle: "outline",
    specs: [
  ...
  },
  {
    ...
    imageRight: false, bg: C.surface, dark: false,
    ctaText: "Acquire Bundle", ctaStyle: "solid",
    specs: [
  ...
  },
  {
    ...
    imageRight: true, bg: C.surfaceContainerHigh, dark: false,
    ctaText: "Acquire Bundle", ctaStyle: "solid",
    specs: [
  ...
  },
  {
    ...
    imageRight: true, bg: C.primary, dark: true,
    ctaText: "Request Access", ctaStyle: "light",
    specs: [
  ...
  },
  {
    ...
    imageRight: true, bg: C.surfaceContainerLowest, dark: false,
    ctaText: "Acquire Bundle", ctaStyle: "solid",
    specs: [
  ...
  },
  {
    ...
    imageRight: true, bg: C.surfaceDim, dark: false,
    ctaText: "Acquire Bundle", ctaStyle: "solid",
    specs: [
...
// ─── Dossier section ──────────────────────────────────────────────────────────
function DossierSection({ d, index }: { d: Dossier; index: number }) {
  ...
  const ctaBorder = d.ctaStyle === "outline" ? `2px solid ${d.dark ? C.white : C.primary}` : "none";

  const textCol = (
    <div
      ref={textRef}
      className={isAlt ? "order-2 md:order-1" : "order-2"}
      style={{
    ...
    >
      ...
      <h2 style={{ ...PLAYFAIR, fontSize: 48, lineHeight: "56px", fontWeight: 600, color: d.dark ? C.white : C.primary, marginBottom: 24 }}>
        {d.title}
      </h2>
      <p style={{ ...INTER, fontSize: 16, lineHeight: "24px", color: d.dark ? "rgba(255,255,255,0.7)" : C.onSurfaceVariant, marginBottom: 40 }}>
        {d.desc}
      </p>

      <div style={{ borderLeft: `1px solid ${d.dark ? "rgba(255,255,255,0.2)" : "rgba(0,22,12,0.2)"}`, paddingLeft: 24, marginBottom: 40, display: "flex", flexDirection: "column", gap: 24 }}>
        {d.specs.map(s => (
      ...
      <button
        ...
        data-testid={`button-dossier-${d.id}`}
      >
        {d.ctaText}
      </button>
    </div>
...
export default function DogGiftSeries() {
  ...
  return (
    <div style={{ backgroundColor: C.surface, color: C.onSurface, overflowX: "hidden" }}>
      ...
      {/* ─── Hero: The Synchronization Vault ─── */}
      ...
      <div>
        {DOSSIERS.map((d, i) => (
          <DossierSection key={d.id} d={d} index={i} />
        ))}
      </div>
