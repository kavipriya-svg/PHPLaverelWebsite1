...
import { Plus, Pencil, Trash2, Upload, X, Play, Image as ImageIcon } from "lucide-react";
...
export default function DogParentClothingTestimonials() {
  ...
  const { data: items = [], isLoading } = useQuery<Testimonial[]>({
    queryKey: ["/api/admin/dog-parent-clothing-testimonials"],
  });
  ...
  const saveMutation = useMutation({
    mutationFn: () => {
      ...
      return editing
        ? apiRequest("PUT", `/api/admin/dog-parent-clothing-testimonials/${editing.id}`, payload)
        : apiRequest("POST", "/api/admin/dog-parent-clothing-testimonials", payload);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/admin/dog-parent-clothing-testimonials"] });
  ...
  const deleteMutation = useMutation({
    mutationFn: (id: string) => apiRequest("DELETE", `/api/admin/dog-parent-clothing-testimonials/${id}`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/admin/dog-parent-clothing-testimonials"] });
      toast({ title: "Deleted" });
    },
    ...
    <AdminLayout>
      <div className="p-6 max-w-4xl mx-auto space-y-6">
        <div className="flex items-center justify-between flex-wrap gap-2">
          <div>
            <h1 className="text-2xl font-bold">Dog Parent Clothing — Testimonials</h1>
            <p className="text-muted-foreground text-sm mt-1">Field Log cards shown in the testimonials section of the twinning page</p>
          </div>
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1">
                ...
                <Input value={form.subjectCode} onChange={e => setForm(f => ({ ...f, subjectCode: e.target.value }))} placeholder="SUBJECT-001" data-testid="input-subject-code" />
              </div>
              <div className="space-y-1">
            ...
            <div className="space-y-1">
              <Label>Quote</Label>
              <Textarea value={form.quote} onChange={e => setForm(f => ({ ...f, quote: e.target.value }))} rows={3} placeholder="Subject feedback…" data-testid="input-quote" />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1">
