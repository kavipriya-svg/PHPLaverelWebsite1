...
export const swimGroomProviders = pgTable("swim_groom_providers", {
  ...
  localityId: varchar("locality_id").references(() => swimGroomLocalities.id),
  pincode: varchar("pincode"),
  gstNumber: varchar("gst_number"),
  gstName: varchar("gst_name"),
  logoUrl: varchar("logo_url"),
  bannerUrl: varchar("banner_url"),
  videoUrl: varchar("video_url"),
...
export const swimGroomProvidersRelations = relations(swimGroomProviders, ({ one, many }) => ({
  ...
  locality: one(swimGroomLocalities, {
    fields: [swimGroomProviders.localityId],
    references: [swimGroomLocalities.id],
  }),
...
export const dogTreatsFeedback = pgTable("dog_treats_feedback", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  name: varchar("name", { length: 120 }).notNull(),
  role: varchar("role", { length: 120 }).notNull().default("Verified Buyer"),
  avatarBg: varchar("avatar_bg", { length: 20 }).notNull().default("#a5d0b8"),
...
export const insertDogTreatsFeedbackSchema = createInsertSchema(dogTreatsFeedback).omit({
  id: true,
  createdAt: true,
});
export type InsertDogTreatsFeedback = z.infer<typeof insertDogTreatsFeedbackSchema>;
export type DogTreatsFeedback = typeof dogTreatsFeedback.$inferSelect;

// ─── Dog Treats Ad Banners ────────────────────────────────────────────────────
export const dogTreatsAdBanners = pgTable("dog_treats_ad_banners", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  title: varchar("title", { length: 255 }),
  subtitle: varchar("subtitle", { length: 500 }),
  ctaText: varchar("cta_text", { length: 100 }),
...
export const insertDogTreatsAdBannerSchema = createInsertSchema(dogTreatsAdBanners).omit({
  id: true,
  createdAt: true,
});
...
export const dogParentClothingTestimonials = pgTable("dog_parent_clothing_testimonials", {
  id:                varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  subjectCode:       varchar("subject_code",       { length: 50  }).notNull().default("SUBJECT"),
  satisfactionLabel: varchar("satisfaction_label", { length: 50  }).notNull().default("5/5"),
  quote:             text("quote").notNull(),
...
export const insertDogParentClothingTestimonialSchema = createInsertSchema(dogParentClothingTestimonials).omit({
  id: true, createdAt: true,
});
export type InsertDogParentClothingTestimonial = z.infer<typeof insertDogParentClothingTestimonialSchema>;
export type DogParentClothingTestimonial = typeof dogParentClothingTestimonials.$inferSelect;

// ─── Dog Parent Clothing Ad Banners ──────────────────────────────────────────
export const dogParentClothingAdBanners = pgTable("dog_parent_clothing_ad_banners", {
  id:        varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  title:     varchar("title",    { length: 255  }),
  subtitle:  varchar("subtitle", { length: 500  }),
  ctaText:   varchar("cta_text", { length: 100  }),
...
export const insertDogParentClothingAdBannerSchema = createInsertSchema(dogParentClothingAdBanners).omit({
  id: true, createdAt: true,
});
